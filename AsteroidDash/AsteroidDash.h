...
public:
    ...
    CelestialObject* deleteCelestialObject(CelestialObject*& head, CelestialObject*& target);

    // Corresponds to the SHOOT command.
    void shoot();

    void resetSpaceGrid();
    void placePlayerOnGrid();
    void placeBulletsOnGrid();
    vector<vector<bool>> rotate_right(const vector<vector<bool>>& shape);
    bool has_only_one_true(const std::vector<std::vector<bool>>& shape);
    void print_game_over(bool is_game_over);
};
